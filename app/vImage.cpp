...
bool VImage::load2DTexture( const std::string& path,
                            VImage& img,
                            VkDevice device,
                            VkFormat format,
                            ...
                            VkQueue queue )
{
   ...
   VMemAlloc hostBuffer = VkUtils::createBuffer(
      ...
      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, img._size,
   ...
   VK_CALL( vkBindImageMemory( device, img._image, img._memAlloc.memory, img._memAlloc.offset ) );
   ...
   vkCmdCopyBufferToImage( cmdBuf, stagingBuffer, img._image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           1, &bufferCopyRegion );
...
bool VImage::loadCubeTexture( const std::string& path,
                              VImage& img,
                              VkDevice device,
                              VkFormat format,
                              VkImageTiling tiling,
                              ...
                              VkQueue queue )
{
   ...
   for ( uint32_t face = 0; face < 6; ++face )
   {
      for ( uint32_t level = 0; level < img._mips; ++level )
      {
         ...
         bufferCopyRegion.imageExtent.width =
            static_cast<uint32_t>( cubeTex[ face ][ level ].extent().x );
         bufferCopyRegion.imageExtent.height =
            static_cast<uint32_t>( cubeTex[ face ][ level ].extent().y );
         bufferCopyRegion.imageExtent.depth = 1;
   ...
   vkCmdCopyBufferToImage(
      commandBuffer, stagingBuffer, img._image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
