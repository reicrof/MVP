...
public:
	...
	uint64_t alloc(uint64_t size, uint64_t alignment)
	{
		...
		if (it != _allocatedChunks.begin())
		{
			...
			// Remove alignment free space if prev block is used
			if (!prevIt->isFree) { _freeSpace -= alignmentPadding;  }
		}
	...
	{ 
		...
		const bool mergeRight = it != _allocatedChunks.end() && (it + 1) != _allocatedChunks.end() &&
			(it + 1)->isFree;
		// Merge left if not first chunk and if previous chunk exists.
		const bool mergeLeft = it != _allocatedChunks.begin() && (it - 1) != _allocatedChunks.begin() &&
			(it - 1)->isFree;
