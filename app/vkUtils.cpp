...
void VkUtils::endCmdBufferAndSubmit( VkCommandBuffer cmdBuf,
                                     VkQueue& queue,
...
VkCommandBuffer VkUtils::copyBuffer( VkCommandBuffer cmdBuffer,
                                     VkBuffer source,
                                     VkBuffer dest,
                                     VkDeviceSize size )
{
...
void VkUtils::createImage( VkDevice device,
                           VMemoryManager& memoryManager,
                           uint32_t width,
                           uint32_t height,
                           uint32_t mips,
                           ...
                           VImage& img )
{
   ...
   VK_CALL( vkBindImageMemory( device, img, img.getMemory().memory, img.getMemory().offset ) );
}

VkCommandBuffer VkUtils::transitionImgLayout( VkImage image,
                                              VkFormat format,
...
VkWriteDescriptorSet VkUtils::createWriteDescriptorSet( VkDescriptorSet dstSet,
                                                        uint32_t dstBinding,
                                                        uint32_t dstArrayEl,
...
VkWriteDescriptorSet VkUtils::createWriteDescriptorSet( VkDescriptorSet dstSet,
                                                        uint32_t dstBinding,
                                                        uint32_t dstArrayEl,
                                                        const VkDescriptorImageInfo* imgInfos,
                                                        uint32_t imgInfosCount )
{
   VkWriteDescriptorSet descriptorWrites = {};
   descriptorWrites.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
   descriptorWrites.dstSet = dstSet;
...
VkFence VkUtils::createFence( VkDevice device, VkFenceCreateFlags flag )
{
   VkFence fence = VK_NULL_HANDLE;
   VkFenceCreateInfo createInfo = {};
   createInfo.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
   createInfo.flags = flag;
   VK_CALL( vkCreateFence( device, &createInfo, nullptr, &fence ) );
   return fence;
}
